...
// ✅ NEW: Enhanced URL validation function
...
// ✅ NEW: GitHub specific validation
const isValidGitHubUrl = (url) => {
  if (!url || url.trim() === "") return true;
  return url.includes('github.com') && isValidUrl(url);
};

// ✅ NEW: Profile data validation
const validateProfileData = (data) => {
  const errors = {};

  // Basic profile validation
  if (data.photoUrl && !isValidUrl(data.photoUrl)) {
    errors.photoUrl = "Profile photo URL must be a valid URL starting with http:// or https://";
  }
  ...
  if (data.projects) {
    data.projects.forEach((project, index) => {
      if (project.githubLink && !isValidGitHubUrl(project.githubLink)) {
        errors[`projects[${index}].githubLink`] = "GitHub link must be a valid GitHub URL";
      }
      if (project.liveLink && !isValidUrl(project.liveLink)) {
        errors[`projects[${index}].liveLink`] = "Live demo link must be a valid URL";
      }
      if (project.image && !isValidUrl(project.image)) {
        errors[`projects[${index}].image`] = "Project image must be a valid URL";
      }
    });
  }
  ...
  if (data.certificates) {
    data.certificates.forEach((cert, index) => {
      if (cert.link && !isValidUrl(cert.link)) {
        errors[`certificates[${index}].link`] = "Certificate link must be a valid URL";
      }
    });
  }
  ...
  if (data.codingProfiles) {
    data.codingProfiles.forEach((profile, index) => {
      if (profile.profileUrl && !isValidUrl(profile.profileUrl)) {
        errors[`codingProfiles[${index}].profileUrl`] = "Profile URL must be a valid URL";
      }
    });
  }
  ...
  if (data.contributions) {
    data.contributions.forEach((contribution, index) => {
      if (contribution.projectLink && !isValidUrl(contribution.projectLink)) {
        errors[`contributions[${index}].projectLink`] = "Project link must be a valid URL";
      }
      if (contribution.certificateLink && !isValidUrl(contribution.certificateLink)) {
        errors[`contributions[${index}].certificateLink`] = "Certificate link must be a valid URL";
      }
    });
  }
